 ...
 *
 */
public class MoblieRechangeRecordParser extends NetParser{
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			switch (eventType) {
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("rechamoney") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
				...
				}else if (parser.getName().compareTo("rechamobile") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("rechamobile", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("rechamobileprov") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("rechamobileprov", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("rechabkcardno") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("rechabkcardno", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("rechadatetime") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("rechadatetime", temp);
						parent.addChild(data);
					}
				}
				else if (parser.getName().compareTo("rechastate") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("rechastate", temp);
						parent.addChild(data);
					}
