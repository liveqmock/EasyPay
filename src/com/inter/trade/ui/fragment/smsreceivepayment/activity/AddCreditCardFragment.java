...
public class AddCreditCardFragment extends BaseFragment implements OnClickListener, IFavoriteCharacterDialogListener{
...
//	private EditText mybk_phone_edit;
	...
	public static AddCreditCardFragment create(Bundle b){
		return new AddCreditCardFragment(b);
	}
	...
	public AddCreditCardFragment(Bundle b)
	{
		this.bundle = b;
	}
...
//		mBankCardData=(MyBankListData) bundle.getSerializable("data");
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.sms_creditcard_add_layout, container,false);
		initView(view);
...
//		setTitle("添加银行卡");
		...
		new BankTask(getActivity(), new ResponseStateListener() {
			...
			public void onSuccess(Object obj, Class cla) {
				...
				bankList=bankListFilter(bankList);
			}
		}, true).execute("");
	 ...
	 * @return
	 */
	private ArrayList<BankData> bankListFilter(ArrayList<BankData> bl){
		String[] sBankArrayList = {"中国工商银行","中国农业银行","中国银行","中国建设银行","中国光大银行","交通银行",};
		boolean flag=true;
		
		ArrayList<BankData> banklistfilter = new ArrayList<BankData>();
		for(BankData bankData : bl) {
			for(String s : sBankArrayList) {
				if(s.equals(bankData.bankname)) {
					flag=false;
					break;
				}
			}
			if(flag){
				banklistfilter.add(bankData);
			}
			flag=true;
		}
		return banklistfilter;
	}
...
//		mBankCardData.bkcardbank = bkname;
		...
		/**
		 * 此处保存的必定是【默认】收款卡，所以传1，跟【我的银行卡】进去的页面不同
		 */
		...
		if(checkInput()){
			...
			new AddBankCardTask2(getActivity(), new ResponseStateListener(){
				...
				public void onSuccess(Object obj,Class cla){
					 ...
					 */
					Intent intent =new Intent();
					intent.putExtra("bankcard", mBankCardData);
...
//					 }
			...
			,"0"
			,"0",mBankCardData.bkcardshoudefault);
		}
	}
