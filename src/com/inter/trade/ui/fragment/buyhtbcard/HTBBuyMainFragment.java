...
import com.inter.trade.adapter.HTBBuyHistoryAdapter;
...
public class HTBBuyMainFragment extends BaseFragment implements OnClickListener,OnItemClickListener {
	...
	HTBBuyHistoryAdapter mAdapter;
	public static final String TYPE_STRING="TYPE_STRING";
//	private String mType;
	public static HTBBuyMainFragment instance(String type){
		HTBBuyMainFragment fragment = new HTBBuyMainFragment();
	...
	public HTBBuyMainFragment() {

	}

	public HTBBuyMainFragment(Bundle b) {
		this.bundle = b;
	}
...
//		startActivity(intent);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.htb_buycard_layout, container, false);
...
//			break;
	...
	private void agentReplenishPay() {
		...
		if("".equals(number)){
			PromptUtil.showToast(getActivity(), "请输入订购数量");
			return;
		}
		mNumber = Integer.parseInt(number);
		if(mNumber <=0){
			PromptUtil.showToast(getActivity(), "订购数量输入不正确");
			return;
		}
		
		if(mNumber > agentData.limitmaxnum && agentData.limitmaxnum>0){
			PromptUtil.showToast(getActivity(), "订购数量不能超过"+agentData.limitmaxnum);
			return;
		}
		if(mNumber < agentData.limitminnum && agentData.limitminnum>0){
			PromptUtil.showToast(getActivity(), "订购数量不能少于"+agentData.limitminnum);
			return;
		}
...
//		startActivityForResult(intent, 3);
		...
		IMainHandlerManager.handlerUI(UIConstantDefault.UI_CONSTANT_ORDER_PAY, 1, bundle);
	}
	...
	public class RecordTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected Boolean doInBackground(String... arg0) {
			...
			try {
				...
				List<ProtocolData> mDatas = ProtocolUtil.getRequestDatas("ApiExpresspayInfo", "readagentorder",
						data);
...
//					agentData.limitminnum = 20;
					...
					mAdapter = new HTBBuyHistoryAdapter(getActivity(), mList);
					mListView.setAdapter(mAdapter);
					mAdapter.notifyDataSetChanged();
