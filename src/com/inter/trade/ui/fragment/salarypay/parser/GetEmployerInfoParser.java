 ...
 *
 */
public class GetEmployerInfoParser extends NetParser {
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("result") == 0) {
					...
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData r = new ProtocolData("name", temp);
						res.addChild(r);
					}
				}else if (parser.getName().compareTo("hasRegister") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData r = new ProtocolData("hasRegister", temp);
						res.addChild(r);
					}
				}else if (parser.getName().compareTo("bankAccount") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData r = new ProtocolData("bankAccount", temp);
						res.addChild(r);
					}
				}
			}
