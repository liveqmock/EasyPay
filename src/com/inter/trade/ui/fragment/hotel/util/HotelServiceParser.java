 ...
 *
 */
public class HotelServiceParser extends NetParser<HotelServiceData>{
	...
	@Override
	public ArrayList<HotelServiceData> parserResponseDatas(
			List<ProtocolData> mDatas, Bundle bundle) {
		ArrayList<HotelServiceData> responseDatas = new ArrayList<HotelServiceData>();
		...
		for (ProtocolData data : mDatas) {
			if(data.mKey.equals(ProtocolUtil.msgheader)){
				...
				for(ProtocolData child:aupic){
					HotelServiceData recordData = new HotelServiceData();
					if (child.mChildren != null && child.mChildren.size() > 0) {
						Set<String> keys = child.mChildren.keySet();
						for(String key:keys){
							List<ProtocolData> rs = child.mChildren.get(key);
							for(ProtocolData item: rs){
								if(item.mKey.equals("service")){
									recordData.service  = item.mValue;
									
								}else if(item.mKey.equals("amenity")){
									recordData.amenity  = item.mValue;
