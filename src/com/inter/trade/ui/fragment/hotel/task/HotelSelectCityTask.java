 ...
 *
 */
public class HotelSelectCityTask extends AsyncTask<String, String, String> {
	...
	public HotelSelectCityTask(Context context, ResponseStateListener listener) {
		super();
		this.context = context;
		this.listener = listener;
	}
	
	public HotelSelectCityTask(Context context, NetParser netParser, SunType data, ResponseStateListener listener) {
	 ...
	 * @param listener
	 */
	public HotelSelectCityTask(Context context, NetParser netParser, SunType data, boolean isbackground, ResponseStateListener listener) {
...
//			mRsp = HttpUtil.doRequest(myParser, mDatas);
	...
	protected void onPostExecute(String result) {
		...
		if(mRsp==null){
			...
			try {
				...
				ArrayList<HotelGetCityData> parserResponse = parserResponse(mDatas);
				listener.onSuccess(parserResponse, HotelGetCityData.class);
	...
	private ArrayList<HotelGetCityData> parserResponse(List<ProtocolData> mDatas) {
		ArrayList<HotelGetCityData> netData=new ArrayList<HotelGetCityData>();
		...
		for (ProtocolData data : mDatas) {
			if (data.mKey.equals(ProtocolUtil.msgheader)) {
				...
				netData=new ArrayList<HotelGetCityData>();
				if(aupic!=null)
				for(ProtocolData child:aupic){
					HotelGetCityData adata=new HotelGetCityData();
					if (child.mChildren != null && child.mChildren.size() > 0) {
						...
						for(String key:keys){
							...
							for(ProtocolData item: rs){
								...
								}else if(item.mKey.equals("cityName")){
