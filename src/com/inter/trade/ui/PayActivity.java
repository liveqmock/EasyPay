...
import com.inter.trade.ui.fragment.order.OrderSuccessFragment;
...
//		LoginUtil.detection(this);
		int index =0;
		index = getIntent().getIntExtra(FragmentFactory.INDEX_KEY, 0);
		FragmentManager fragmentManager = getSupportFragmentManager();
		FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
		fragmentTransaction.add( R.id.func_container, new PayFactory().createFragment(index,null));
	...
	private void payResult(Intent data){
        ...
        new BuySuccessTask(this, resultData,"ApiOrderInfo","orderPayFeedback").execute("");
        ...
        builder.setNegativeButton("确定", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                if (str.equalsIgnoreCase("success")) {
	                	 FragmentTransaction transaction = PayActivity.this.getSupportFragmentManager().beginTransaction();
	 	        		transaction.replace(R.id.func_container,OrderSuccessFragment.createFragment(mCommonData));
	 	        		transaction.commit();
                }
            }
        });
