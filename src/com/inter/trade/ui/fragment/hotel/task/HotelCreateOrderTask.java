 ...
 * @version:  V1.0
 */
public class HotelCreateOrderTask extends AsyncTask<String, Void, Void> {
	...
	public HotelCreateOrderTask(Context context, ResponseStateListener listener, CommonData hotelData) {
		super();
	...
	protected Void doInBackground(String... params) {
			...
			try {
				List<ProtocolData> mDatas = ProtocolUtil.getRequestDatas(
						"ApiHotel", "CreateOrder", hotelData);
				HotelCreateOrderParser authorRegParser = new HotelCreateOrderParser();
		...
		private SmsCode parserResponse(List<ProtocolData> mDatas) {
			...
			for (ProtocolData data : mDatas) {
				if (data.mKey.equals(ProtocolUtil.msgheader)) {
					...
					List<ProtocolData> money = data.find("/money");
					if(money!=null){
						smscode.setMoney(money.get(0).mValue);
					}
				}
			}
			return smscode;
