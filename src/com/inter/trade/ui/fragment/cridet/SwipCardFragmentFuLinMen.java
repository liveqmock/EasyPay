...
import com.inter.trade.util.KeyboardUtil;
...
public class SwipCardFragmentFuLinMen extends BaseFragment implements OnClickListener,SwipListener, SmsCodeSubmitListener{
	 ...
	 * 银行卡密码
	 */
	private EditText bank_card_pwd;
	...
	public SwipCardFragmentFuLinMen()
	{
	}
...
//		initReader();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// TODO Auto-generated method stub
		view = inflater.inflate(R.layout.cridet_fulinmen_pay_layout, container,false);
		initView(view);
	...
	private void getCreditCardMoneyPayfee () {
	 ...
	 private AsyncLoadWorkListener getCreditCardMoneyPayfeeListener = new AsyncLoadWorkListener() {
		...
		public void onSuccess(Object protocolDataList, Bundle bundle) {
			...
			if(fee != null) {
				//更新UI
				repay_money.setText(fee.allmoney+"元");
...
//	}
	...
	private void showKeyboardPopupWindow () {
		RelativeLayout keyboard = (RelativeLayout)LayoutInflater.from(getActivity()).inflate(
				R.layout.default_keyboard_layout, null);
		
		popupWindow = new PopupWindow(getActivity());
		popupWindow.setBackgroundDrawable(new BitmapDrawable());
		popupWindow.setWidth(getActivity().getWindowManager().getDefaultDisplay().getWidth() );
		popupWindow.setHeight(getActivity().getWindowManager().getDefaultDisplay().getWidth());
...
//		});
		...
		popupWindow.showAtLocation(keyboard, Gravity.RIGHT | Gravity.BOTTOM , 0, keyboard.getHeight());//需要指定Gravity，默认情况是center.
		
		new KeyboardUtil(getActivity(), keyboard, bank_card_pwd).showKeyboard();
	}
	...
	private void initView(final View view){
		...
		//自定义键盘
		bank_card_pwd = (EditText)view.findViewById(R.id.bank_card_pwd);
		bank_card_pwd.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				disableSysKeyboard(bank_card_pwd);
				new KeyboardUtil(getActivity(), getActivity(), bank_card_pwd).showKeyboard();
			}
		});
		bank_card_pwd.setOnFocusChangeListener(new OnFocusChangeListener() {
			
			@Override
			public void onFocusChange(View v, boolean hasFocus) {
				Log.i("key onFocusChange", hasFocus+"");
				if(hasFocus) {//当获取焦点的时候显示自定义键盘，同时禁止系统键盘
					disableSysKeyboard(bank_card_pwd);
					new KeyboardUtil(getActivity(), getActivity(), bank_card_pwd).showKeyboard();
				}
			}
		});
//		bank_card_pwd.setInputType(InputType.TYPE_NULL);
		bank_card_pwd.setOnTouchListener(new OnTouchListener() {
			@Override
			public boolean onTouch(View v, MotionEvent event) {
				disableSysKeyboard(bank_card_pwd);
				int inputback = bank_card_pwd.getInputType();
				bank_card_pwd.setInputType(InputType.TYPE_NULL);
				bank_card_pwd.onTouchEvent(event);
//				showKeyboardPopupWindow ();
				
//				ScrollView scrollView = (ScrollView)view;
//				        scrollView.fullScroll(ScrollView.FOCUS_DOWN);
				bank_card_pwd.setInputType(inputback);
				return true;
			}
		});
	...
	public void onClick(View arg0) {
		...
		case R.id.cridet_back_btn:
			...
			if(checkInput()){
				...
				PromptUtil.showToast(getActivity(), "正在检测");
			}
		...
		public void onSuccess(Object protocolDataList, Bundle bundle) {
			...
			if (mSmsCode != null) {
				...
				if (mSmsCode.isNeed()) {// 需要验证码
					...
					dialog.show(getActivity(), SwipCardFragmentFuLinMen.this);
				} else {// 不需要验证码,表示支付成功
...
//		}
		...
		String cardpwd = bank_card_pwd.getText().toString();
		if(null == cardpwd || "".equals(cardpwd)){
			PromptUtil.showToast(getActivity(), "请输入银行卡密码");
			return false;
		}
		
		if(cardpwd.length() != 6){
			PromptUtil.showToast(getActivity(), "请输入6位银行卡密码");
			return false;
		}
		
		CridetCardFragment.mJournalData.putValue("cardpwd", cardpwd);
