 ...
 *
 */
public class CouponListParser extends NetParser{
	...
	public ProtocolData parserBody(XmlPullParser parser)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG: {
				...
				}else if (parser.getName().compareTo("shopname") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData r = new ProtocolData("shopname", temp);
						res.addChild(r);
					}
				}else if (parser.getName().compareTo("isshop") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData r = new ProtocolData("isshop", temp);
						res.addChild(r);
					}
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG: {
				...
				}else if (parser.getName().compareTo("couponlimitnum") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("couponlimitnum", temp);
						parent.addChild(data);
					}
				}
			}
