...
import com.inter.trade.ui.fragment.salarypay.parser.AddEmployerInfoParser;
 ...
 * @version:  V1.0
 */
public class AddEmployerInfoTask extends AsyncTask<EmployerData, Void, Void> {
	
	private Context context;
	private ResponseStateListener listener;
	
	public AddEmployerInfoTask(Context context, ResponseStateListener listener) {
		super();
	...
	protected Void doInBackground(EmployerData... params) {
		...
		try {
			...
			mData.putValue("month",employer.month);
			...
			List<ProtocolData> mDatas = ProtocolUtil.getRequestDatas("ApiWageInfo", 
					"AddSalaryItem", mData);
			AddEmployerInfoParser myParser = new AddEmployerInfoParser();
