 ...
 * @version:  V1.0
 */
public class PayGetTask extends AsyncTask<String, Void, Void> {
	
	private Context context;
	private ResponseStateListener listener;
	
	public PayGetTask(Context context, ResponseStateListener listener) {
		super();
		...
		if(mRsp==null){
			...
			try {
				...
				if(!ErrorUtil.create().errorDeal(LoginUtil.mLoginStatus,(Activity)context)){
					if(listener!=null && listener instanceof ResponseMoreStateListener){
						((ResponseMoreStateListener)listener).onFailure(message, String.class);
					}
					return;
				}
				if(listener!=null){
