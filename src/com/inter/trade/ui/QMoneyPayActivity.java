...
import com.inter.trade.ui.fragment.qmoney.QMoneyPayConfirmFragment;
 ...
 *
 */
public class QMoneyPayActivity extends FragmentActivity{
...
//		LoginUtil.detection(this);
		...
		if(bundle != null){
			transaction.replace(R.id.func_container, new QMoneyPayConfirmFragment(bundle));
		}else{
			transaction.replace(R.id.func_container, new QMoneyPayConfirmFragment());
		}
		transaction.commit();
	}
	...
	private void payResult(Intent data){
        ...
        new BuySuccessTask(this, resultData,"ApiQQRechangeInfo","checkRechaMoneyStatus").execute("");
        ...
        builder.setNegativeButton("确定", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                if (str.equalsIgnoreCase("success")) {
	                	 FragmentTransaction transaction = QMoneyPayActivity.this.getSupportFragmentManager().beginTransaction();
	 	        		transaction.replace(R.id.func_container,QMoneyPaySuccessFragment.createFragment(mCommonData));
	 	        		transaction.commit();
                }else{
                		finish();
                }
