...
import com.inter.trade.ui.creditcard.task.DeleteBankCardTask;
...
//	private TextView tv_bankname, tvCard;
...
//	private EditText mybk_phone_edit;
	...
	public static MyBankCreditCardEditFragment create(Bundle b){
		return new MyBankCreditCardEditFragment(b);
	}
	...
	public MyBankCreditCardEditFragment(Bundle b)
	{
		this.bundle = b;
	}
...
//			et_bankname.setText(mBankCardData.bkcardbank);
			...
			if(mBankCardData.bkcardyxmonth.length()==1){
				mBankCardData.bkcardyxmonth = "0" + mBankCardData.bkcardyxmonth;
			}
			if(mBankCardData.bkcardyxyear.length()==4){
				mBankCardData.bkcardyxyear =mBankCardData.bkcardyxyear.substring(2);
			}
			 ...
			 * 默认支付卡(兼容旧接口)
			 */
			if(mBankCardData.bkcardisdefault != null){
				if("1".equals(mBankCardData.bkcardisdefault)){
			 ...
			 * 默认支付卡
			 */
			if(mBankCardData.bkcardfudefault != null){
				if("1".equals(mBankCardData.bkcardfudefault)){
					cb_default.setChecked(true);
				}else{
					cb_default.setChecked(false);
				}
			 ...
			 * 默认收款卡
			 */
			if(mBankCardData.bkcardshoudefault != null){
				if("1".equals(mBankCardData.bkcardshoudefault)){
					cb_default_receive.setChecked(true);
				}else{
					cb_default_receive.setChecked(false);
				}
			}
		}
	}
...
//		mBankCardData.bkcardbank = bkname;
	...
	private void myBankDelete()
	{
		if(true){
			//task
			new DeleteBankCardTask(getActivity(), new ResponseStateListener(){
			...
			).execute(mBankCardData.bkcardid+"");
		}
	}
