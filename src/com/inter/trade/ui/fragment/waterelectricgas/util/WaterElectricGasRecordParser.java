 ...
 *
 */
public class WaterElectricGasRecordParser extends NetParser{
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			switch (eventType) {
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("bkntno") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("bkntno", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("factNumber") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("factNumber", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("payNumber") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("payNumber", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("company") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("company", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("proId") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("proId", temp);
						parent.addChild(data);
					}
