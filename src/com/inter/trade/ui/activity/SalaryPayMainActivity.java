...
import com.inter.trade.ui.fragment.salarypay.SalaryEmployeeEditFragment;
import com.inter.trade.ui.fragment.salarypay.SalaryMainFragment;
import com.inter.trade.ui.fragment.salarypay.SalaryPayConfirmFragment;
import com.inter.trade.ui.fragment.salarypay.SalaryPayHistoryFragment;
import com.inter.trade.ui.fragment.salarypay.SalaryPayListConfirmFragment;
import com.inter.trade.ui.fragment.salarypay.SalaryPayMainFragment;
...
/**
 * 工资签收 UI Activity 管理类
 * 
 * @author zhichao.huang
 * 
 */
public class SalaryPayMainActivity extends FragmentActivity implements
		IMainHandlerProcess,ResponseStateListener {

	public static final String TAG = SalaryPayMainActivity.class
			.getSimpleName();
	...
	private Fragment switchUI(int uitag, Bundle data) {
		...
		case UIConstantDefault.UI_CONSTANT_SALARYPAY_MAIN:

			fragment = SalaryPayMainFragment.getInstance(data);

			break;

		case UIConstantDefault.UI_CONSTANT_SALARYPAY_LISTCONFIRM:

			fragment = SalaryPayListConfirmFragment.getInstance(data);

			break;

		case UIConstantDefault.UI_CONSTANT_SALARYPAY_HISTORY:

			fragment = new SalaryPayHistoryFragment();

			break;
		case UIConstantDefault.UI_CONSTANT_SALARYPAY_CONFIRM:

			fragment = SalaryPayConfirmFragment.getInstance(data);

			break;
		case UIConstantDefault.UI_CONSTANT_SALARYPAY_SUCCESS:

			fragment =SalaryPaySuccessFragment.getInstance(data);
		...
		case UIConstantDefault.UI_CONSTANT_SALARY_MAIN:

			fragment = new SalaryMainFragment();

			break;

		case UIConstantDefault.UI_CONSTANT_SALARY_EMPLOYEE_EDIT:

			fragment = SalaryEmployeeEditFragment.getInstance(data);

			break;
			
		case UIConstantDefault.UI_CONSTANT_SALARY_CREDITPAY:
			fragment = SalaryCommonEasyCreditcardPayFragment.newInstance(data);
			break;
	...
	private void payResult(Intent data) {
		...
		if (str.equalsIgnoreCase("success")) {
			...
			handlerUI(UIConstantDefault.UI_CONSTANT_SALARYPAY_SUCCESS, 1, bundle);
