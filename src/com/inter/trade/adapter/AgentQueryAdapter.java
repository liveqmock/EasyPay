 ...
 *
 */
public class AgentQueryAdapter extends BaseExpandableListAdapter{
	private Context mContext;
	private LayoutInflater mInflater;
	private Resources mResources;
	private ArrayList<AgentQueryData> mGroupList;
	private ArrayList<ArrayList<AgentQueryDetailData>> mChildList;
...
//	String[][] str_child_items_2 = {{}, {"200.00", "200.00", "28.00"}, {"4280.00"}};
	...
	public AgentQueryAdapter(Context context, ArrayList<AgentQueryData> groupList, ArrayList<ArrayList<AgentQueryDetailData>> childList){
...
//	return str_child_items_[groupPosition].length;
	return (mChildList.size()>0 &&  mChildList.size()>groupPosition)? mChildList.get(groupPosition).size() : 0;
}
...
//	mHolder.money.setText("￥"+str_child_items_2[groupPosition][childPosition]);
	mHolder.title.setText(mChildList.get(groupPosition).get(childPosition).appfunname);
	mHolder.money.setText("￥"+mChildList.get(groupPosition).get(childPosition).fenrun);
...
//	mHolder.money.setText("￥"+str_group_items_2[groupPosition]);
	mHolder.title.setText(mGroupList.get(groupPosition).appfunname);
