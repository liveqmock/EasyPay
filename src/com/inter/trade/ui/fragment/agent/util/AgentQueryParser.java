 ...
 *
 */
public class AgentQueryParser extends NetParser{
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("result") == 0) {
					...
					res.addChild(r);
				}else if (parser.getName().compareTo("totalfenrun") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData r = new ProtocolData("totalfenrun", temp);
						res.addChild(r);
					}
				}
			}
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			switch (eventType) {
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("appfunid") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("appfunid", temp);
				...
				}else if (parser.getName().compareTo("allfenrun") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("allfenrun", temp);
						parent.addChild(data);
					}
				}
			}
