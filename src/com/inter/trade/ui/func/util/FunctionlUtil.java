...
public class FunctionlUtil {
	...
	public static final int FIRSTPAGE_COUNT = 6;// 我的最爱显示个数

	/*** 记录默认的功能项目的id集合 */
	private static ArrayList<Integer> favourKeyList = new ArrayList<Integer>();//
	private static ArrayList<FuncData> favour = new ArrayList<FuncData>();// 我的最爱
	private static ArrayList<ArrayList<FuncData>> bank = new ArrayList<ArrayList<FuncData>>();// 掌上银行
	private static ArrayList<ArrayList<FuncData>> convince = new ArrayList<ArrayList<FuncData>>();// 便民服务
	...
	private static void initFunctionMap() {
		...
		cridetData.imageId = R.drawable.icon_visa;
		...
		transferData.imageId = R.drawable.icon_transfer;
		...
		couponData.imageId = R.drawable.icon_coupon;
		...
		expressData.imageId = R.drawable.icon_expressage;
		...
		balanceData.imageId = R.drawable.icon_balance;
		...
		telephoneData.imageId = R.drawable.icon_mobilecharge;
		...
		qMoneyData.imageId = R.drawable.icon_qq;
		...
		agentData.imageId = R.drawable.icon_agent_main;
		// agentData.name = "代理商";
		...
		buySwipCardData.imageId = R.drawable.icon_buy;
		...
		waterElectricGasData.imageId = R.drawable.iocn_waterfee;
		...
		gameRechargeData.imageId = R.drawable.icon_game;
		...
		airTicketData.imageId = R.drawable.icon_plane;
		...
		trainTicketData.imageId = R.drawable.icon_train;
		...
		hotelData.imageId = R.drawable.icon_hotel;
		// hotelData.name = "酒店预订";

		// 通付卡
		FuncData tfbCard = new FuncData();
		tfbCard.mnuid = PreferenceConfig.instance(PayApp.pay).getInt(
				FuncMap.TFBCARD_FUNC_KEY, FuncMap.TFBCARD_INDEX_FUNC);
		tfbCard.imageId = R.drawable.icon_tfbcard;

		// 发工资
		FuncData salary = new FuncData();
		salary.mnuid = PreferenceConfig.instance(PayApp.pay).getInt(
				FuncMap.SALARYPAY_FUNC_KEY, FuncMap.SALARYPAY_INDEX_FUNC);
		salary.imageId = R.drawable.icon_slaray;

		//工资签收
//		FuncData salaryget = new FuncData();
//		salaryget.mnuid = PreferenceConfig.instance(PayApp.pay).getInt(
//				FuncMap.SALARYGET_FUNC_KEY, FuncMap.SALARYGET_INDEX_FUNC);
//		salaryget.imageId = R.drawable.icon_hotel;
		
		// 苏宁商城
		FuncData suningmall = new FuncData();
		suningmall.mnuid = PreferenceConfig.instance(PayApp.pay).getInt(
				FuncMap.SUNINGMALL_FUNC_KEY, FuncMap.SUNINGMALL_INDEX_FUNC);
		suningmall.imageId = R.drawable.icon_suning_shoping;
		
		//储蓄卡还信用卡（付临门通道）
		FuncData savingscard = new FuncData();
		savingscard.mnuid = PreferenceConfig.instance(PayApp.pay).getInt(
				FuncMap.SAVINGSCARD_FUNC_KEY, FuncMap.SAVINGSCARD_INDEX_FUNC);
		savingscard.imageId = R.drawable.icon_visa;
		
		//短信收款
		FuncData smsData = new FuncData();
		smsData.mnuid = PreferenceConfig.instance(PayApp.pay).getInt(
				FuncMap.SMSRECEIPT_FUNC_KEY, FuncMap.SMSRECEIPT_INDEX_FUNC);
		smsData.imageId = R.drawable.icon_sms_receipt;
		...
		functionList.put(FuncMap.SMSRECEIPT_INDEX_FUNC, smsData);
		...
		functionMap.put(FuncMap.FAMILY, waterElectricGasData);
		...
		functionMap.put(FuncMap.TFBCARD, tfbCard);
		functionMap.put(FuncMap.SALARYPAY, salary);
		//functionMap.put(FuncMap.SALARYGET, salaryget);
		functionMap.put(FuncMap.SUNINGMALL, suningmall);
		functionMap.put(FuncMap.SAVINGSCARD, savingscard);
		functionMap.put(FuncMap.SMSRECEIPT, smsData);
	}
	 ...
	 */
	public static void getFunctions(ModleData modle) {
		initFunctionMap();
		...
		for (IconData icon : mList) {// 获取菜单的名字
			FuncData findFun = findFun(icon.getMnuno());
			findFun.name = icon.getMnuname();
			functionMap.put(icon.getMnuno(), findFun);
		}
		...
		if (record.size() > 0) {// 用户开始记录使用习惯
			if (record.size() >= FIRSTPAGE_COUNT) {// 功能的使用个数为6
				favourList.clear();
				for (int i = 0; i < record.size(); i++) {
					if (i == 6)
						break;
					favourList.add(recordList.get(i));
				}
			} else {// 功能的记录少于9个
				int size = favourList.size();
//				System.out
//						.println("-----------------------favour-size:" + size);
...
//				System.out.println("-----------------------favour-size:"
//						+ favourList.size());
//				System.out.println("-----------------------record-size:"
//						+ recordList.size());
				if (favourList.size() + recordList.size() > FIRSTPAGE_COUNT) {// 当前两者的和大于6，删除默认首页的最后几直到两者之和为6
					int fsize = favourList.size();
					int rsize = recordList.size();
//					System.out.println("当前默认功能" + fsize + "当前记录" + rsize);
					int deletesize = fsize + rsize - FIRSTPAGE_COUNT;
			...
			if (favourList.size() > 6) {
				ArrayList<FuncData> temp = subList(favourList, 0, 6);
				favourList = temp;
			}
		}
		...
		// 各种分类的列表
		HashMap<String, ArrayList<ArrayList<FuncData>>> funMap = new HashMap<String, ArrayList<ArrayList<FuncData>>>();// 以mnutypeid为key
			...
			for (String key : map.keySet()) {
				...
				if (totalSize <= DEFAULT_COUNT) {// 小于每页显示个数
					...
					if (totalSize % DEFAULT_COUNT == 0) {// 功能数刚好是默认的倍数
						...
						for (int i = 1; i <= page; i++) {
							if (i == page) {
								ArrayList<FuncData> subList = subList(
										defaultList, start, defaultList.size());
								resultList.add(subList);
							} else {
								ArrayList<FuncData> subList = subList(
										defaultList, start, start
												+ DEFAULT_COUNT);
								resultList.add(subList);
							}
							start += DEFAULT_COUNT;
				...
				funMap.put(key, resultList);
			}
//			System.out.println("获取的菜单的页数:" + funMap.size());
			for (String key : funMap.keySet()) {
				ArrayList<ArrayList<FuncData>> arrayList = funMap.get(key);
//				System.out.println("获取的菜单的第一页的个数:" + arrayList.get(0).size());
				// TODO 正式环境需要修改对应的Id,如果是测试环境是3；正式：1
				if ("1".equals(key)) {
					bank = arrayList;
				} else {
					convince = arrayList;
	 ...
	 * @return ArrayList<FuncData>
	 */
	public static ArrayList<FuncData> getFavour() {
		return favour;
	}
	 ...
	 * @return ArrayList<ArrayList<FuncData>>
	 */
	public static ArrayList<ArrayList<FuncData>> getBank() {
		return bank;
	}
	 ...
	 * @return ArrayList<ArrayList<FuncData>>
	 */
	public static ArrayList<ArrayList<FuncData>> getConvince() {
		return convince;
	}
