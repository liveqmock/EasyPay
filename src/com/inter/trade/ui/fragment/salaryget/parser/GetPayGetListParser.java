package com.inter.trade.ui.fragment.salaryget.parser;
 ...
 *
 */
public class GetPayGetListParser extends NetParser {
		...
		if (body.mChildren != null && body.mChildren.size() > 0) {
			...
			for(String key:keys){
				...
				for(ProtocolData data: rs){
					...
					}else if(data.mKey.equals("wageid")){
						serializer.startTag("", "wageid");
						serializer.text(data.mValue.trim());
						serializer.endTag("", "wageid");
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			switch (eventType) {
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("wagemoney") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
				...
				}else if (parser.getName().compareTo("isqianshou") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("isqianshou", temp);
						parent.addChild(data);
					}
				}else if (parser.getName().compareTo("wageid") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("wageid", temp);
						parent.addChild(data);
					}
				}
			}
