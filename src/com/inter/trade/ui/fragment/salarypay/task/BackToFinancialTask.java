...
import com.inter.trade.ui.fragment.salarypay.parser.BackToFinancialParser;
 ...
 * @version:  V1.0
 */
public class BackToFinancialTask extends AsyncTask<String, Void, Void> {
	
	private Context context;
	private ResponseStateListener listener;
	
	public BackToFinancialTask(Context context, ResponseStateListener listener) {
		super();
	...
	protected Void doInBackground(String... params) {
		try {
			...
			mData.putValue("changecwcanEdit",params[2]);//1：可以修改  0：财务将不可以修改 
			
			List<ProtocolData> mDatas = ProtocolUtil.getRequestDatas("ApiWageInfo", 
					"payWagecwedit", mData);
			BackToFinancialParser myParser = new BackToFinancialParser();
	...
	protected void onPreExecute() {
		super.onPreExecute();
		PromptUtil.showDialog(context, "请稍后...");
	}
	@Override
		...
		for (ProtocolData data : mDatas) {
			if (data.mKey.equals(ProtocolUtil.msgheader)) {
				...
				if (result != null) {
					...
					PromptUtil.showToast(context,message.get(0).getmValue());
				}
			}
		}
	}
