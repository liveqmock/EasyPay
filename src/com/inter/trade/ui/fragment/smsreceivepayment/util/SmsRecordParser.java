 ...
 *
 */
public class SmsRecordParser extends NetParser{
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			switch (eventType) {
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("fumobile") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("fumobile", temp);
				...
				}else if (parser.getName().compareTo("memo") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("memo", temp);
						parent.addChild(data);
					}
				...
				else if (parser.getName().compareTo("smsrid") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("smsrid", temp);
						parent.addChild(data);
					}
				}
				else if (parser.getName().compareTo("smsrdate") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("smsrdate", temp);
						parent.addChild(data);
					}
				}
				else if (parser.getName().compareTo("smsrstate") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("smsrstate", temp);
						parent.addChild(data);
					}
				}
			}
