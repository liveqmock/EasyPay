 ...
 *
 */
public class AgentAreaParser extends NetParser{
	...
	private  void parserHelpItem(XmlPullParser parser, ProtocolData parent)
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			switch (eventType) {
			case XmlPullParser.START_TAG: {
				if (parser.getName().compareTo("prov") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("prov", temp);
				...
				}else if (parser.getName().compareTo("town") == 0) {
					String temp = parser.nextText();
					if (temp != null && temp.length() > 0) {
						ProtocolData data = new ProtocolData("town", temp);
						parent.addChild(data);
					}
				}
			}
