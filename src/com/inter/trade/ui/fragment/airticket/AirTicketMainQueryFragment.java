...
/**
 * 飞机票 机票查询(去程) Fragment
 * @author zhichao.huang
 *
 */
public class AirTicketMainQueryFragment extends IBaseFragment implements OnClickListener,OnItemClickListener{

	private static final String TAG = AirTicketMainQueryFragment.class.getName();
	...
	public static AirTicketMainQueryFragment newInstance (Bundle data) {
		AirTicketMainQueryFragment fragment = new AirTicketMainQueryFragment();
	...
	private void initViews (View rootView) {
		...
		//设置当前查询的日期
		calendar.setText(bundleData != null ? bundleData.getString("departDate") : "");
...
//			mListView.setLastText();
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if(bundleData != null && airlineData != null) {
			...
			bundleData.putSerializable("AirlineData", airlineData);//去程航班信息,如果内字段airticketFanchengAirlineData 不为空，说明存在返程信息
		}
		
		//用户选的是往返机票查询
		if(bundleData.getString("searchType").equals("D")) {
			addFragmentToStack(UIConstantDefault.UI_CONSTANT_AIR_TICKET_QUERY_FANCHENG, 1, bundleData);
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.before_day://前一天
			if(DateUtil.compareDate(calendar.getText().toString())) {
				PromptUtil.showToast(getActivity(), "在当天之前的日期不能选择");
			}else{
...
//			/**
//			 * 用户选的是返程，点击后一天的时候，去程的日期不能大于返程的日期
//			 */
//			if(bundleData.getString("searchType") != null && bundleData.getString("searchType").equals("D")) {
//				
//				
//				
//			}
	...
	public class AirTicketQueryTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected Boolean doInBackground(String... arg0) {
			...
			try {
				...
				data.putValue("departDate", airTicketDate != null && !airTicketDate.equals("") ? airTicketDate : bundleData.getString("departDate"));
				data.putValue("returnDate", ""/**bundleData.getString("returnDate")*/);
				data.putValue("searchType", "S"/**bundleData.getString("searchType")*/);
		...
		protected void onPostExecute(Boolean result) {
			...
			try {
				if (mRsp == null) {
					...
					//测试返程
					mList = AirLineUtils.getAirlineDatas(mList, bundleData.getString("departCityCode"), 1);
